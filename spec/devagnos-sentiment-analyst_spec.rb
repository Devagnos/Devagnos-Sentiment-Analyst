require_relative '../lib/devagnos-sentiment-analyst'

describe Mentalist do
  let(:root) { File.expand_path(File.join(File.dirname(__FILE__), '..')) }
  let(:loader) { Mentalist.new(threshold: 0.1) }
  ...
  let(:analyzer) { Mentalist.new(threshold: 0.1) }
  before :each do
  ...
  describe 'n-grams' do
    let(:word_scores) { nil }
    subject do
      Mentalist.new(word_scores: word_scores, ngrams: 3)
    end
